    ...
    for code in code_list:
        while True:
            try:
                ...
                pass
        if len(temp) >= 2 and temp.iloc[-1]['시가'] != 0 and temp.iloc[-2]['시가'] != 0:
...
def _transaction(date, candidate_code_list, balance, **transaction_params):
    ...
    allowed_minutes = transaction_params['매수 및 매도 허용 시간 (단위: 분)']
    ...
    """
    is_sat = True
    for time_idx in range(60):
        if minute_data[time_idx]:
            is_sat = False
            break
    starting_idx = 60 if is_sat else 0

    """
    거래량 부족 종목 리스트
    """
    day_transaction_short_code_list = []
    ...
    if sell_all_flag:
        ...
        for stock in stock_list:
            quantity = stock['수량']
            # 최대 allowed_minutes 매도
            last_price = 0
            stock_starting_idx = starting_idx
            while True:
                price, volumes = get_price_volumes(minute_data=minute_data, time_idx=stock_starting_idx,
                                                   code=stock['코드'])
                if price == 0 or volumes == 0:
                    stock_starting_idx += 1
                else:
                    break
            for time_idx in range(stock_starting_idx, stock_starting_idx + allowed_minutes):
                ...
                # 거래량 부족한 경우:
                    ...
                    quantity -= volumes
            if last_price == 0:
                print('거래량 부족으로 인한 시뮬레이션 불가 (종목 코드: ' + stock['코드'] + ')')
                sys.exit(0)
            if quantity != 0:
                day_transaction_short_code_list.append(stock['코드'])
                balance.sell_stock(sell_date=date, sell_time=timestamp[time_idx], code=stock['코드'],
                                   sell_price=last_price, sell_quantity=quantity, is_stock=is_stock)
    ...
    if use_maturity and sell_param_list_after_maturity[0] == '익일 시가':
        ...
        for stock in stock_list:
            if stock['보유일수'] >= maturity:
                ...
                stock_starting_idx = starting_idx
                while True:
                    price, volumes = get_price_volumes(minute_data=minute_data, time_idx=stock_starting_idx, code=stock['코드'])
                    if price == 0 or volumes == 0:
                        stock_starting_idx += 1
                    else:
                        break
    ...
    * 만기 사용 시 재 매수 금지
    ...
    if not allow_repurchase or use_maturity:
        candidate_code_list = [code for code in candidate_code_list if code not in balance.get_all_stock_code_list()]

    """
    분 단위 매매 시뮬레이션
    """
    today_bought_code_list = []
    for time_idx in range(starting_idx, len(timestamp)):
        # 매수
                ...
                if code not in today_bought_code_list:
                    ...
                    if price <= target_purchase_price and not price >= target_sell_price:
                        ...
                        if price <= break_even_price:
                            ...
                            # 최대 allowed_minutes 매수
                            bought = False
                            for p_time_idx in range(time_idx, min(time_idx+allowed_minutes, len(timestamp)-1)):
            ...
            if use_maturity:
                ...
                if stock['보유일수'] < maturity:
                    ...
                    if price >= win_price or price <= lose_price:
                        ...
                        # 최대 allowed_minutes 매도
                        last_price = 0
                        for s_time_idx in range(time_idx, min(time_idx+allowed_minutes, len(timestamp)-1)):
                    ...
                    if price >= target_sell_price:
                        ...
                        if quantity != 0:
                            day_transaction_short_code_list.append(stock['코드'])
                            balance.sell_stock(sell_date=date, sell_time=timestamp[s_time_idx], code=stock['코드'], sell_price=last_price,
                                               sell_quantity=quantity, is_stock=is_stock)
    ...
    if use_maturity and sell_param_list_after_maturity[0] == '당일 종가':
        ...
        for stock in stock_list:
            if stock['보유일수'] >= maturity - 1:
                ...
                # 최대 allowed_minutes 매도
                last_price = 0
                for time_idx in range(len(timestamp)-allowed_minutes-1, len(timestamp)):
                    price, volumes = get_price_volumes(minute_data=minute_data, time_idx=time_idx, code=stock['코드'])
                    # 거래 없는 경우
                    if price == 0 or volumes == 0:
                        continue
    ...
    return day_stock_list, day_sold_stock_list, day_transaction_history, asset_change, _yield, asset, day_transaction_short_code_list
